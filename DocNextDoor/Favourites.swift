...
struct Favourites: View {
    ...
    var body: some View {
        ZStack { //background
            ...
            VStack { //full view
                VStack { //Header
                    //Rectangle().frame(width: 600, height: 50, alignment: .center).foregroundColor(Color("BackG")).padding(5)
                    Text("Annonces sauvegardées").bold().font(.title2).padding(20)
                    ...
                    if (profilUser.adsFav != nil) { //si on a des favoris
                            ...
                            ForEach(profilUser.adsFav!) {  ad in //affichage en liste
                                HStack {
                                    ...
                                    VStack { //stroke Color = "\(ad.speciality)Color"
                ...
                if (pickerList == 1) { //picker
                    if (profilUser.adsApplied != nil) { //si on a des favoris
                            List {
                            ForEach(profilUser.adsFav!) {  ad in //affichage en liste
                                HStack {
                                    NavigationLink(destination: DetailledAd()){ //add parametre (ad: ad) --> DetailledAd((ad: ad))
                                ...
                                Button() {
                                    print("Annonce retirée des candidatures")
                                } label: {
                                    Label("Supprimer", systemImage: "trash.fill")
                                }.tint(.red)
                                Button () {
                                    ...
                                    Label("Candidatée", systemImage: "heart")
                                }
                                .tint(Color("Lightblue"))//fin Hstack 1 element de la list
                            }//fin For Each
                            }.frame(width: 400, height: 700, alignment: .center) //fin list
                        }//fin if
                    else { //si pas de candidature
                        VStack {
                            Spacer()
                            Image("NoFav").resizable().frame(width: 280, height: 240, alignment: .center).padding(30)
                            Text("Vous n'avez encore candiaté à aucune annonce : continuez d'explorer les opportunités !").foregroundColor(Color("Txtgrey")).font(.body).italic().frame(width: 335, height: 70, alignment: .center).multilineTextAlignment(.center)
                            Spacer()
                            Spacer()
                        }//fin Vstack illu + txt no favorite
                    }//fin else
